...
Basal_Area_09m = (sum((pi*(DBH[real.dist <= 9]/20)^2)/10000)/(pi*9^2))*10000,
Basal_Area_08m = (sum((pi*(DBH[real.dist <= 8]/20)^2)/10000)/(pi*8^2))*10000,
Basal_Area_07m = (sum((pi*(DBH[real.dist <= 7]/20)^2)/10000)/(pi*7^2))*10000,
Basal_Area_06m = (sum((pi*(DBH[real.dist <= 6]/20)^2)/10000)/(pi*6^2))*10000,
Basal_Area_05m = (sum((pi*(DBH[real.dist <= 5]/20)^2)/10000)/(pi*5^2))*10000,
Basal_Area_04m = (sum((pi*(DBH[real.dist <= 4]/20)^2)/10000)/(pi*4^2))*10000)
...
Broad_BA_09m = (sum((pi*(DBH[real.dist <= 9]/20)^2)/10000)/(pi*9^2))*10000,
Broad_BA_08m = (sum((pi*(DBH[real.dist <= 8]/20)^2)/10000)/(pi*8^2))*10000,
Broad_BA_07m = (sum((pi*(DBH[real.dist <= 7]/20)^2)/10000)/(pi*7^2))*10000,
Broad_BA_06m = (sum((pi*(DBH[real.dist <= 6]/20)^2)/10000)/(pi*6^2))*10000,
Broad_BA_05m = (sum((pi*(DBH[real.dist <= 5]/20)^2)/10000)/(pi*5^2))*10000,
Broad_BA_04m = (sum((pi*(DBH[real.dist <= 4]/20)^2)/10000)/(pi*4^2))*10000)
...
broad.ba2 <- tree.data.plot2[,which(names(tree.data.plot2) %in% c("Site", "Plot", "Broad_BA_20m", "Broad_BA_19m", "Broad_BA_18m", "Broad_BA_17m", "Broad_BA_16m", "Broad_BA_15m", "Broad_BA_14m", "Broad_BA_13m", "Broad_BA_12m", "Broad_BA_11m", "Broad_BA_10m", "Broad_BA_09m", "Broad_BA_08m", "Broad_BA_07m", "Broad_BA_06m", "Broad_BA_05m", "Broad_BA_04m"))]

broad.ba3 <- tree.data.plot3[,which(names(tree.data.plot3) %in% c("Site", "Plot", "Broad_BA_20m", "Broad_BA_19m", "Broad_BA_18m", "Broad_BA_17m", "Broad_BA_16m", "Broad_BA_15m", "Broad_BA_14m", "Broad_BA_13m", "Broad_BA_12m", "Broad_BA_11m", "Broad_BA_10m", "Broad_BA_09m", "Broad_BA_08m", "Broad_BA_07m", "Broad_BA_06m", "Broad_BA_05m", "Broad_BA_04m"))]
...
tree.data.plot$perc_broad_09m <- tree.data.plot$Broad_BA_09m/tree.data.plot$Basal_Area_09m*100
tree.data.plot$perc_broad_08m <- tree.data.plot$Broad_BA_08m/tree.data.plot$Basal_Area_08m*100
tree.data.plot$perc_broad_07m <- tree.data.plot$Broad_BA_07m/tree.data.plot$Basal_Area_07m*100
tree.data.plot$perc_broad_06m <- tree.data.plot$Broad_BA_06m/tree.data.plot$Basal_Area_06m*100
tree.data.plot$perc_broad_05m <- tree.data.plot$Broad_BA_05m/tree.data.plot$Basal_Area_05m*100
tree.data.plot$perc_broad_04m <- tree.data.plot$Broad_BA_04m/tree.data.plot$Basal_Area_04m*100
...
```{r}
tree.data.plot$site_label <- ifelse(tree.data.plot$site == "Arville", "B (50° N)", ifelse(tree.data.plot$site == "Hochwald", "G (49° N)", ifelse(tree.data.plot$site == "Vivarp", "SS (56° N)", ifelse(tree.data.plot$site == "Siljan", "MS (60° N)", "NS (64° N)"))))

tree.data.plot %>%
# filter(!(site %in% "Arville")) %>%
 ggplot() +
 aes(x = Basal_Area_20m,
     y = perc_broad_20m, colour = site_label) +
 geom_point(aes(shape = site_label), 
 size = 2.5) +
  scale_shape_manual(name = "Site",
                     values=c(15, 16, 17, 18, 20),
                     limits=c("NS (64° N)", "MS (60° N)", "SS (56° N)", "B (50° N)", "G (49° N)")) +
  scale_color_manual(name = "Site",
                     values=c("#E09F3E", "#89BDB9", "#335C67", "#65875A", "#9E2A2B"),
                     limits=c("NS (64° N)", "MS (60° N)", "SS (56° N)", "B (50° N)", "G (49° N)")) +
 labs(x = expression(paste("Basal Area (", m^2, " \U00B7 ", ha^-1,")")), y = "Broadleaves (%)", fill = "Site") +
 theme_minimal() +
