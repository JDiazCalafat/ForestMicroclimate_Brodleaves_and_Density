...
#
# to be able to use Hemiphot.R yo will need to install the functions
...
### Location parameters
location.latitude   = 60.834
location.altitude   = 194
location.day        = 76
...
location.cx         = 3000             # x coordinate of center
location.cy         =  2000             # y coordinate of center
location.cr         =  1650             # radius of circle
...
all.images = list.files("Hemispherical_Pictures/Siljan/",pattern = ".JPG")
...
for(i in 1:nr.images){    
  ## read file
  image = readJPEG(paste("Hemispherical_Pictures/Siljan/",all.images[i],sep = ""), native = F)     #if native = T creates a raster, else an array
...
# save data
write.csv2(all.data, "HemiphotOutput.csv", row.names = FALSE)
