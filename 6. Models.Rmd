...
# Prev_av_prec is not significant (0.6). Let's remove it:

model.meanair.jan.ba.noprec <- gamm4(air_off ~ s(Basal_Area_20m,perc_broad_20m) + s(Day) + site + wind + snow + prev_av_temp, random = ~ (1|site:Plot), data = tree.temp.a.jan)

summary(model.meanair.jan.ba.noprec$gam)
AIC(model.meanair.jan.ba.noprec$mer)
...
model.meanair.jan.co.nosnowprec <- gamm4(air_off ~ s(CanOpen,perc_broad_20m) + s(Day) + site + wind + snow + prev_av_temp, random = ~ (1|site:Plot), data = tree.temp.a.jan)
