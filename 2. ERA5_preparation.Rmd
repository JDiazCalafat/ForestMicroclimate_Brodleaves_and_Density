In this script, we will compile macroclimate data from different sources (ERA5 downloaded directly from the web, ERA5 downloaded through the mcera5 package and weather stations found close to our study sites). We will produce one file as output for each of these sources. Additional files with below-24h resolution data will also be exported to be used in script 2.5
...
Extract ERA5 mean temperature raw data for Script 2.5

```{r}
sequence <- as.data.frame(seq(as.POSIXct('2020-01-01 00:00:00'), as.POSIXct('2021-07-31 23:00:00'), by = 'hour'))

names(sequence) <- "seq"

sequence <- separate(sequence, seq, into = c("Date", "time"), sep = " (?=[^ ]+$)")

era5.air.temp$time <- rep(sequence$time, length.out = length(era5.air.temp$date))

era5.air.temp$date <- gsub(".", "-", era5.air.temp$date, fixed = TRUE)

write.csv(era5.air.temp, file = "./DATASETS/ERA5_raw_hour.csv", row.names = FALSE, quote = FALSE)
...
snow <- aggregate(list(era5_snow = snow$snow), by = list(Date = snow$date, site = snow$site), FUN = sum)
...
Format "Date" and turn it into POSIXct class.
...
```{r}
uid <- "109976"
cds_api_key <- "c88eab9f-cb84-4237-99ce-ef2f12bcfcfc"
```
...
en_time2020 <-  lubridate::ymd("2020:07:31")
...
# Vivarp
   # Warmest month (air): 2021
   # Coldest month (air): 2021
...
write.csv(mcera5, file = "./DATASETS/mcera5_raw.csv", quote = FALSE, row.names = FALSE) # save a version of this dataset for plots in script 2.5
...
arville.prec <- rbind(arville.warm.prec, arville.cold.prec)
...
hochwald.prec <- rbind(hochwald.warm.prec, hochwald.cold.prec)
...
vivarp.prec <- rbind(vivarp.warm.prec, vivarp.cold.prec)
rm(vivarp.warm.prec)
rm(vivarp.cold.prec)
...
siljan.prec <- rbind(siljan.warm.prec, siljan.cold.prec)
rm(siljan.warm.prec)
rm(siljan.cold.prec)
...
sandsjo.prec <- rbind(sandsjo.warm.prec, sandsjo.cold.prec)
rm(sandsjo.warm.prec)
rm(sandsjo.cold.prec)
```

Merge precipitation data aross al sites:
...
hochwald.station.hourly <- hochwald.station # backup of daily data for future graphs
...
vivarp.dates <- c(seq(as.Date("2019-12-25"),as.Date("2020-01-31"), by = 'day'), seq(as.Date("2021-06-25"),as.Date("2021-07-31"), by = 'day'))
...
sandsjo.dates <- seq(as.Date("2020-06-25"),as.Date("2021-01-31"), by = 'day')
...
Save data from weather stations with temperature values at a resolution lower than 24h for script 2.5

```{r}
#missing columns
hochwald.station.hourly$site <- "Hochwald"
hochwald.station.hourly$station_minair <- NA
hochwald.station.hourly$station_maxair <- NA
vivarp.station.hourly$station_minair <- NA
vivarp.station.hourly$station_maxair <- NA

weather.stations.hourly <- rbind(arville.station.hourly, hochwald.station.hourly, vivarp.station.hourly)

write.csv(weather.stations.hourly, file = "./DATASETS/ws_hourlydata.csv", quote = FALSE, row.names = FALSE)
```
