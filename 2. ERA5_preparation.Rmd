...
```
And subset the ERA5 data to our climatic extent (earliest date: 2019-09-01; latest date: 2021-07-03).

```{r}
air.temp.nc <- air.temp.nc[[which(getZ(air.temp.nc) >= as.Date("2019-09-01") & getZ(air.temp.nc) <= as.Date("2021-07-03"))]]

max.temp.nc <- max.temp.nc[[which(getZ(max.temp.nc) >= as.Date("2019-09-01") & getZ(max.temp.nc) <= as.Date("2021-07-03"))]]

min.temp.nc <- min.temp.nc[[which(getZ(min.temp.nc) >= as.Date("2019-09-01") & getZ(min.temp.nc) <= as.Date("2021-07-03"))]]

soil.temp.nc <- soil.temp.nc[[which(getZ(soil.temp.nc) >= as.Date("2019-09-01") & getZ(soil.temp.nc) <= as.Date("2021-07-03"))]]

wind.nc <- wind.nc[[which(getZ(wind.nc) >= as.Date("2019-09-01") & getZ(wind.nc) <= as.Date("2021-07-03"))]]
```
...
wind <- aggregate(list(wind = wind$wind), by = list(Date = wind$date, site = wind$site), FUN = min)
```
...
```{r}
library(reshape)
era5.temp <- merge_recurse(list(era5.air.temp, era5.max.temp, era5.min.temp, era5.soil.temp, wind))
```
